    ...
    <div className={cn("flex min-h-0 flex-col", className)}>
      <div className="border-line flex items-start gap-3 border-b px-5 pt-[18px] pb-3.5">
        ...
        <div className="min-w-0">
          ...
          {subtitle ? <p className="text-ink-3 mt-0.5 text-label">{subtitle}</p> : null}
        </div>
      </div>
...
function Section({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <section className={cn("border-line border-b px-5 py-3.5 last:border-b-0", className)}>
      <h2 className="text-ink-3 mb-2.5 text-caption font-medium tracking-[0.1em] uppercase"><TranslatedText>{label}</TranslatedText></h2>
      {children}
    </section>
  );
}
    ...
    <dl className="grid grid-cols-2 gap-x-3.5 gap-y-2.5">
      {items.map((f) => (
        <div key={f.label} className="min-w-0">
          <dt className="text-ink-3 text-caption"><TranslatedText>{f.label}</TranslatedText></dt>
