    ...
    <div className="mx-auto w-full max-w-[1480px] space-y-5 pb-2">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="bg-surface border-line text-ink-2 inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs">
          <ShieldCheck className="text-brand size-3.5" aria-hidden="true" />{ROLE_LABEL[actor.role]}
          <span className="text-line-strong" aria-hidden="true">/</span>{yearLabel} BS
        </span>
      </header>
      ...
      <section className="relative isolate overflow-hidden rounded-2xl bg-brand-deep px-6 py-7 text-white sm:px-8 sm:py-8">
        ...
        <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
          <div className="min-w-0 max-w-2xl">
            <p className="flex items-center gap-2 text-xs font-medium text-white/75"><CalendarDays className="size-3.5" aria-hidden="true" />{formatBs(o.today.date, "dddd, DD MMMM YYYY")}</p>
            <h2 className="font-display mt-3 break-words text-[28px] font-semibold leading-tight tracking-tight sm:text-[34px]">Welcome back, {actor.username}.</h2>
            <p className="mt-2 max-w-xl text-sm leading-6 text-white/75">{teacher ? "Your classes, your students, and a little more room to focus on teaching." : office ? "Keep the school day moving. Your registers, records, and daily tasks are all here." : "A clear picture of your school, with the details that need your attention."}</p>
            {primary ? <Link href={primary.href} className="mt-5 inline-flex min-h-10 items-center gap-3 rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-[var(--brand-deep)] transition-colors hover:bg-white/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-brand-deep">{primary.label}<ArrowRight className="size-4" aria-hidden="true" /></Link> : null}
          </div>
          <div className="w-full rounded-xl border border-white/15 bg-white/[0.06] p-4 lg:w-64 lg:shrink-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-white/65">{roll.due ? "Today’s focus" : "A quieter day"}</p>
            <p className="mt-2 text-lg font-semibold">{!roll.due ? "No roll call due" : missing > 0 ? `${missing} ${missing === 1 ? "class needs" : "classes need"} roll call` : teacher ? "Make time for your classes" : "Ready for the day"}</p>
            <p className="mt-1 text-xs leading-5 text-white/70">{!roll.due ? "Today is a non-working day or outside the selected academic year. You can still review your records." : missing > 0 ? "Start with the attendance registers still waiting to be marked." : "Use your shortcuts below to pick up where you left off."}</p>
          </div>
        </div>
      </section>
      ...
      {teacher && (actor.staffId === null || c.sections === 0) ? (
        <div className="bg-brand-tint border-brand-tint-2 flex items-start gap-3 rounded-xl border p-4">
          ...
          <div><h2 className="text-sm font-semibold">{actor.staffId === null ? "Let’s connect your teaching profile" : "Your classes will appear here"}</h2>
            <p className="text-ink-2 mt-1 text-sm leading-5">{actor.staffId === null ? "Ask your administrator to link this account to your staff record. Your timetable and classes will then appear here." : "You have no visible classes for this academic year. Your administrator can check your teaching assignments and access."}</p>
          </div>
        </div>
      ) : null}

      {metrics.length ? <section aria-label="At a glance" className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        {metrics.map(({ label, value, note, icon: Icon }) => (
          <div key={label} className="bg-surface border-line rounded-xl border p-4 sm:p-5">
            <div className="flex items-center justify-between gap-2"><p className="text-ink-2 text-xs font-medium">{label}</p><Icon className="text-brand size-4" aria-hidden="true" /></div>
            ...
            <p className="text-ink-3 mt-2 text-[11px] leading-4">{note}</p>
          </div>
        ))}
      </section> : null}
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside aria-label="Your tools and insights" className="min-w-0 space-y-5">
          <section className="bg-surface border-line rounded-xl border">
            <div className="border-line border-b px-5 py-4"><h2 className="text-sm font-semibold">Quick access</h2><p className="text-ink-3 mt-1 text-xs">Your everyday tools, one step away.</p></div>
            <div className="space-y-1 p-4">
              {actions.length ? actions.map(({ icon: Icon, ...action }) => (
                <Link key={action.href} href={action.href} className={cn("group hover:bg-surface-2 flex min-h-16 items-center gap-3 rounded-lg px-2 py-3 transition-colors", focusRing)}>
                  ...
                  <span className="min-w-0 flex-1"><span className="block text-sm font-medium">{action.label}</span><span className="text-ink-3 mt-0.5 block text-xs">{action.detail}</span></span>
                  <ArrowRight className="text-ink-3 size-4 shrink-0 transition-transform group-hover:translate-x-0.5" aria-hidden="true" />
                </Link>
              )) : <p className="text-ink-3 py-3 text-sm leading-6">Your administrator can enable the tools you need for your work.</p>}
            </div>
          </section>
          ...
          <div className="border-line rounded-xl border border-dashed px-5 py-4">
            <p className="text-ink-2 flex items-center gap-2 text-xs font-semibold"><ShieldCheck className="text-brand size-4" aria-hidden="true" />Your workspace</p>
            <p className="text-ink-3 mt-2 text-xs leading-5">{teacher ? "This overview only includes classes you teach or lead in the selected year." : office ? "Your office overview brings together the school records and operations your account can manage." : "Your administrator overview covers school operations, staffing, and setup for the selected year."}</p>
          </div>
        </aside>
      </div>
    </div>
