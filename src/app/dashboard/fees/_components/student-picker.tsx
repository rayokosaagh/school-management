    ...
    <div className="min-w-0 space-y-4">
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem]">
        <div className="space-y-2">
          <Label htmlFor={`${id}-search`}>Find a student</Label>
        ...
        <div className="space-y-2">
          <Label htmlFor={`${id}-section`}>Class / section</Label>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-ink-3 text-xs" role="status">{selectedIds.length} selected · {filtered.length} matching student{filtered.length === 1 ? "" : "s"}</p>
        {selectedIds.length > 0 ? <Button type="button" variant="ghost" size="xs" onClick={() => onSelectionChange([])} disabled={disabled}>Clear selection</Button> : null}
      </div>
      <div className="border-line overflow-hidden rounded-xl border">
        <Table>
          <caption className="sr-only">{multiple ? "Select students" : "Choose one student"}. Search and class filters keep existing selections.</caption>
          <TableHeader className="bg-surface-2">
            <TableRow className="hover:bg-transparent">
              <TableHead scope="col" className="w-12 px-4"><span className="sr-only">Select</span></TableHead>
              <TableHead scope="col" className="text-ink-3 text-xs">Student</TableHead>
              <TableHead scope="col" className="text-ink-3 pr-4 text-xs">Class / section</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {visible.map(student => (
            ...
            {visible.length === 0 ? <TableRow><TableCell colSpan={3} className="px-4 py-9 text-center whitespace-normal"><Users className="text-brand mx-auto mb-3 size-6" aria-hidden="true" /><p className="text-sm font-medium">{students.length ? "No students match" : "No students available"}</p><p className="text-ink-3 mt-2 text-xs leading-5">{students.length ? "Try another name, admission number, or class." : "Add active students to this academic year first."}</p>{students.length ? <Button type="button" variant="ghost" size="sm" className="mt-3" onClick={() => { setSearch(""); setSection(""); setPage(0); }} disabled={disabled}>Clear filters</Button> : null}</TableCell></TableRow> : null}
          </TableBody>
        </Table>
      ...
      {multiple ? <p className="text-ink-3 text-xs leading-5">Selections are kept when you search, change class, or move between pages.</p> : null}
    </div>
  );
}
