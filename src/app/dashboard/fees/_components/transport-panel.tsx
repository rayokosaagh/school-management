  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="space-y-2">
        <Label htmlFor={`${id}-location`}>Pickup location</Label>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="min-w-0 space-y-2">
          <Label htmlFor={`${id}-amount`}>Monthly price (Rs.)</Label>
        ...
        <div className="min-w-0 space-y-2">
          <Label htmlFor={`${id}-start`}>Start month (BS)</Label>
      ...
      <div className="bg-surface-2 border-line min-w-0 rounded-xl border p-5">
        <label className="flex cursor-pointer items-start gap-3" htmlFor={`${id}-active`}>
          ...
          <span><span className="block text-sm font-medium">Transport is active</span><span id={`${id}-active-note`} className="text-ink-3 mt-1 block text-xs leading-5">Uncheck to pause new transport bills for this student.</span></span>
        </label>
      </div>
      <p className="text-ink-3 text-xs leading-5">Saving a registration does not issue a bill. Price, location, or status changes leave existing bills unchanged.</p>
      {state.error && state.enrollmentId === enrollmentId ? <p role="alert" className="text-bad text-sm">{state.error}</p> : null}
      <Button type="submit" disabled={pending || !enrollmentId} className="w-full">{pending ? "Saving…" : registration ? "Save transport changes" : "Register for transport"}</Button>
    </form>
  );
}
...
function TransportBilling({ academicYearId, data, onBusy }: { academicYearId: number; data: TransportData; onBusy: (busy: boolean) => void }) {
  ...
  return (
    <form action={action} className="bg-surface-2 border-line min-w-0 space-y-5 rounded-xl border p-5">
      <input type="hidden" name="academicYearId" value={academicYearId} />
      <div><h4 className="flex items-center gap-2 text-sm font-semibold"><Send className="text-brand size-4" aria-hidden="true" />Issue transport bills</h4><p className="text-ink-3 mt-2 text-xs leading-5">Only active registered students are included, from their start month. Students already billed for the month are skipped.</p></div>
      <div className="space-y-2">
        <Label htmlFor={`${id}-month`}>Billing month (BS)</Label>
      ...
      <Button type="submit" disabled={pending || !started || !available} className="w-full">{pending ? "Issuing…" : "Issue transport bills"}</Button>
      {!started ? <p className="text-ink-3 text-xs leading-5">Billing opens when the selected month begins.</p> : !available ? <p className="text-ink-3 text-xs leading-5">No unbilled active registrations for this month.</p> : null}
      {state.error ? <p role="alert" className="text-bad text-sm">{state.error}</p> : null}
      <p className="text-ink-3 text-xs leading-5">Existing bills stay unchanged. Transport is billed separately from class fees.</p>
    </form>
  );
}
      ...
      cell: ({ row }) => (
        <div className="min-w-0">
          ...
          <p className="text-ink-3 text-xs">
            {row.original.billedMonths.length} {row.original.billedMonths.length === 1 ? "month" : "months"} billed
          </p>
        </div>
      ),
    },
    ...
    <section className="min-w-0" aria-label="Transportation service">
            ...
            return (
              <>
                ...
                <Button variant="outline" size="sm" onClick={() => edit(String(registration.enrollmentId))} disabled={pending || !studentActive}>
                  <Pencil data-icon="inline-start" aria-hidden="true" />Edit<span className="sr-only"> transport for {registration.name}</span>
                </Button>
              </>
            );
          }}
      ...
      <Sheet open={open} onOpenChange={(nextOpen, details) => { if (pending) { details.cancel(); return; } setOpen(nextOpen); }}>
        <SheetContent showCloseButton={!pending} className="overflow-y-auto data-[side=right]:w-full data-[side=right]:sm:max-w-2xl">
          <SheetHeader className="px-6 pt-6"><SheetTitle>{selected ? "Edit transport registration" : "Register for transport"}</SheetTitle><SheetDescription>{selectedStudent ? "Set this student's pickup location and monthly transport price." : "Find and select a student to register for transport."}</SheetDescription></SheetHeader>
            ...
            {selectedStudent ? (
              <>
                <div className="bg-brand-tint border-brand-tint-2 flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4">
                  ...
                  <Button type="button" variant="outline" size="sm" disabled={pending} onClick={() => setEnrollmentId("")}>Change student</Button>
                </div>
