  ...
  return (
    <form action={action}>
      ...
      <Button type="submit" variant="outline" size="sm" disabled={pending}>
        {pending ? "Updating…" : isActive ? "Stop using" : "Use again"}
  ...
  return (
    <Sheet>
      <SheetTrigger render={<Button variant="outline" />}>
        ...
        <span className="hidden sm:inline">Manage fee types</span>
        <span className="sm:hidden">Fee types</span>
      </SheetTrigger>
      <SheetContent className="overflow-y-auto data-[side=right]:w-full data-[side=right]:sm:max-w-lg">
        <SheetHeader className="px-6 pt-6">
          <SheetTitle>Fee types</SheetTitle>
          <SheetDescription>Choose class-wide fees or per-student services such as Library. Class fees appear in Class pricing; selected-student fees are configured in Services.</SheetDescription>
        </SheetHeader>
        <div className="space-y-6 px-6 pb-6">
          <section className="border-line bg-surface-2 rounded-xl border p-4">
            <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold"><Plus className="text-brand size-4" aria-hidden="true" />Add a fee type</h3>
            <AddHeadForm />
          </section>
          <section>
            <h3 className="mb-3 text-sm font-semibold">Your fee types <span className="text-ink-3 font-normal">({managedHeads.length})</span></h3>
            {managedHeads.length ? <ul className="divide-line divide-y">{managedHeads.map(head => (
              <li key={head.id} className="flex flex-wrap items-center justify-between gap-3 py-4">
                <Badge variant="outline">{head.billingScope === "STUDENT" ? "Per-student service" : "Class-wide"}</Badge>
                <div className="min-w-0"><p className="break-words text-sm font-medium">{head.name}</p><p className="text-ink-3 mt-1 text-xs">{head.frequency === "MONTHLY" ? "Every month" : "Once a year"} · {head.isActive ? "In use" : "Not in use"}{head._count.invoiceLines > 0 ? ` · on ${head._count.invoiceLines} bill${head._count.invoiceLines === 1 ? "" : "s"}` : head._count.structureLines > 0 ? ` · priced in ${head._count.structureLines} ${head._count.structureLines === 1 ? "class" : "classes"}` : " · never used"}</p></div>
            ...
            ))}</ul> : <p className="text-ink-3 text-sm leading-6">No fee types yet. Add your first one above to start setting class amounts.</p>}
          </section>
        </div>
      </SheetContent>
    </Sheet>
