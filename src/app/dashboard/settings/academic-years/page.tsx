...
import { RolloverWorkspace } from "../../rollover/_components/rollover-workspace";

export default async function RolloverPage() {
  // Redirects unless the stored permission matrix allows this section.
  await requirePage("/dashboard/settings/academic-years");
  ...
  return (
    <RolloverWorkspace
      ...
      years={years
        .filter((y) => Number(y.nameBS) > Number(currentYear.nameBS))
        .map((y) => ({
          id: y.id,
          nameBS: y.nameBS,
          sections: y.sections,
