...
export default async function ActivityPage({ searchParams }: { searchParams: Promise<{ before?: string }> }) {
  ...
  return (
    <PageFrame icon={<History />} tint="blue" eyebrow="Settings" title="Activity history"
      ...
      breadcrumb={<><Link href="/dashboard/settings?view=data" className="hover:underline">Settings</Link> / Activity history</>}>
      <PageFrameBody className="overflow-y-auto p-4">
        <p className="text-ink-2 mb-4 text-sm">This log cannot be edited through the application. It is not a complete security log or a substitute for database backups.</p>
        <Table>
          <TableHeader><TableRow><TableHead>When (Nepal)</TableHead><TableHead>Staff account</TableHead><TableHead>Action</TableHead><TableHead>Record</TableHead><TableHead>Details</TableHead></TableRow></TableHeader>
          <TableBody>
            ...
            {shown.length === 0 ? <TableRow><TableCell colSpan={5}>No recorded activity on this page.</TableCell></TableRow> : null}
          </TableBody>
        </Table>
        <nav aria-label="Activity pages" className="mt-4 flex gap-6 text-sm">
          {before ? <Link href="/dashboard/settings/activity" className="text-brand-text underline">Newest activity</Link> : null}
          {rows.length > 50 ? <Link href={`?before=${shown[shown.length - 1].id}`} className="text-brand-text underline">Older activity</Link> : null}
        </nav>
      </PageFrameBody>
    </PageFrame>
  );
